app_vnm.factory('ctk_withdrawalOfOrders', function($http, $translate) {
	// Khoi tao interface
	return {
		initListStock : function(callback) {
			var url = eim_url + "/bs/Stock/getAllListStockExport";
		...
		searchGoodOnStock : function(stockId, goodName, gNotes, listGoodsOnStock) {
			$http.post(eim_url + '/bs/Stock/getGoodsOnStock?stockId=' + stockId + '&goodName=' + goodName + '&gNotes=' + gNotes).success(listGoodsOnStock).error(
					function(callback) {
						App.unblockUI("#withdrawalOfOrders");
						bootbox.alert($translate.instant('vnm.messages.validate.ProductOrder.All.SV-0010'));
					});
		},
		searchGoodsDetailOnStock : function(stockId, shopId, sourceGood, internalStockId, goodName, goodCode, goodIssueNo, fromSerial, toSerial, gQuantity,
				gState, gNotes, listGoodDetailOnStock) {
			$http.post(
					eim_url + '/bs/Stock/getGoodsDetailOnStock?stockId=' + stockId + '&shopId=' + shopId + '&sourceGood=' + sourceGood + '&internalStockId='
							+ internalStockId + '&goodName=' + goodName + '&goodCode=' + goodCode + '&goodIssueNo=' + goodIssueNo + '&fromSerial=' + fromSerial
							+ '&toSerial=' + toSerial + '&gQuantity=' + gQuantity + '&gState=' + gState + '&gNotes=' + gNotes).success(listGoodDetailOnStock)
					.error(function(callback) {
						App.unblockUI("#withdrawalOfOrders");
						bootbox.alert($translate.instant('vnm.messages.validate.ProductOrder.All.SV-0010'));
					});
		},
		exportForStaff : function(shopId, parentStaffId, parentStaffName, receiptNo, notes, deliveryStockId, internalStockId, orderId, goodTempList,
				resultExport) {
			$http.post(
					eim_url + '/bs/Stock/withdrawalOfOrders?' + '&shopId=' + shopId + '&parentStaffId=' + parentStaffId + '&parentStaffName=' + parentStaffName
							+ '&receiptNo=' + receiptNo + '&notes=' + notes + '&deliveryStockId=' + deliveryStockId + '&internalStockId=' + internalStockId
							+ '&orderId=' + orderId, goodTempList).success(resultExport).error(function(callback) {
				App.unblockUI("#withdrawalOfOrders");
				bootbox.alert($translate.instant('vnm.messages.validate.ProductOrder.All.SV-0010'));
			});
		},
...
app_vnm.controller('ctrl-withdrawalOfOrders', function($scope, $rootScope, $translate, $compile, $timeout, $uibModal, $location, $window, FileUploader,
		$filter, NgTableParams, $localStorage, $element, ctk_withdrawalOfOrders) {
	...
	$scope.model.tab52Reason = 'Thu hồi hàng về kho VNM';
	...
	var tm = '';
	App.blockUI({
		target : "#withdrawalOfOrders",
		boxed : true,
		message : 'loading...'
	});
	ctk_withdrawalOfOrders.initListStock(function(result) {
	...
	$scope.getListGoodsOnStock = function(e) {
		...
		$scope.listGoods = [];
		$scope.model.goods = "";
		$scope.listTab52GoodName = [];
		$scope.model.stockId = "";
		var stockId = '';
		if (e == undefined) {
			stockId = '';
		} else {
			stockId = StringUtilNVLWithDefault(e.stockId, "");
		}
		ctk_withdrawalOfOrders.searchGoodOnStock(stockId, '', '', function(listGoodOnStock) {
			$scope.model.stockId = stockId;
			if (listGoodOnStock.length > 0) {
				$scope.listTab52GoodName = listGoodOnStock;
				$scope.model.tab52GoodName = listGoodOnStock[0].goods_name;
				$scope.model.Tab52goodCode = listGoodOnStock[0].goods_code;
				$scope.listGoodName = listGoodOnStock;
			} else {
				$scope.listTab52GoodName = [];
				$scope.listGoodName = [];
				$scope.model.tab52GoodName = '';
				$scope.model.Tab52goodCode = '';
			}
			App.unblockUI("#withdrawalOfOrders");
		});
	...
	ctk_withdrawalOfOrders.getListOfInternalStock(function(result) {
		...
		ctk_withdrawalOfOrders.searchInventory(StringUtilNVLWithDefault(tm, 'AL'), result[0].internalStockId, StringUtilNVL($scope.model.commodityName),
		...
		/*
		 * ctk_withdrawalOfOrders .searchGoodOnStock( $localStorage.clientContext.shop.shopId, $localStorage.clientContext.shop.shopId, '', '', 'AL',
		 * $scope.model.internalStock, '', function(listGoodOnStock) { if (listGoodOnStock.length > 0) { $scope.listTab52GoodName = listGoodOnStock; //
		 * $scope.model.tab52GoodName=listGoodOnStock[0].goods_name; $scope.listGoodName = listGoodOnStock; } });
		 */
	});
	...
	$scope.searchGoods = function() {
		...
		ctk_withdrawalOfOrders.searchGoodOnStock($localStorage.clientContext.shop.shopId, StringUtilNVLWithDefault($scope.model.stockStaff, ""),
	...
	$scope.searchInventoryTab52 = function() {
		...
		ctk_withdrawalOfOrders.searchGoodsDetailOnStock($scope.model.stockId, $localStorage.clientContext.shop.shopId, StringUtilNVLWithDefault(
				tab52GoodSource, 'AL'), $scope.model.tab52ListInternalStock, StringUtilNVLWithDefault($scope.model.tab52GoodName, ""),
				StringUtilNVLWithDefault($scope.model.Tab52goodCode, ""), '', // ReceiptNo
				...
				function(listGoodsDetailOnStock) {
					if (StringUtilNVLWithDefault(listGoodsDetailOnStock.status, '1') != '1') {
						App.unblockUI("#withdrawalOfOrders");
						var contentMsg = $translate.instant('vnm.messages.validate.inventoryMsg.transferShopNonHierarchical.' + listGoodsDetailOnStock.code);
	...
	$scope.exportInventory = function() {
		...
		if ($scope.frm_withdrawalOfOrders.validate()) {
			if (StringUtilNVLWithDefault($scope.model.tab52StockStaff, '') !== '') {
				if ($scope.stToStockTrans.length > 0) {
					...
					if ($scope.stToStockTrans.length > 0) {
						ctk_withdrawalOfOrders.exportForStaff($localStorage.clientContext.shop.shopId, $localStorage.clientContext.shop.staffId,
								$localStorage.clientContext.shop.staffName, StringUtilNVLWithDefault($scope.model.staffBillId, ""), StringUtilNVLWithDefault(
										$scope.model.notes, ""), $scope.model.tab52StockStaff, $scope.model.tab52ListInternalStock, '10',
								$scope.stToStockTrans, function(resultExport) {
									if (StringUtilNVLWithDefault(resultExport.status, '1') != '1') {
										App.unblockUI("#withdrawalOfOrders");
										var contentMsg = $translate.instant('vnm.messages.validate.inventoryMsg.withdrawalOfOrders.' + resultExport.code);
										...
										resultExport.forEach(function(item, index) {
											if (StringUtilNVLWithDefault(item.exeIEStaffResult, '1') != '1'
												...
												item.notes = $translate.instant('vnm.messages.validate.inventoryMsg.withdrawalOfOrders.' + item.notes);
											}
										...
										bootbox.alert($translate.instant('vnm.messages.validate.inventoryMsg.withdrawalOfOrders.SUCCESS'));
										// tao moi ma phieu
										$scope.stToStockTrans = [];
										$scope.model.staffBillId = new Date().getTime();
									}
						...
						bootbox.alert($translate.instant('vnm.messages.validate.inventoryMsg.withdrawalOfOrders.selected.require'));
						return;
					}

				} else {
					App.unblockUI("#withdrawalOfOrders");
					bootbox.alert($translate.instant('vnm.messages.validate.inventoryMsg.withdrawalOfOrders.detail.required'));
				}
			} else {
				App.unblockUI("#withdrawalOfOrders");
				var element = document.getElementById("showErrorStockStaff");
				$scope.stockStaffMsgError = $translate.instant('vnm.messages.validate.inventoryMsg.withdrawalOfOrders.required');
				element.focus();
			}
		} else {
	...
	$scope.addGoodsForTransfer = function() {
		if ($scope.stStockSelected.length <= 0) {
			var contentMsg = $translate.instant('vnm.messages.validate.inventoryMsg.withdrawalOfOrders.selected.null');
			bootbox.alert(contentMsg);
			...
			$scope.model.addSelected = $scope.stStockSelected.length;
			$scope.tblStToStockData = new NgTableParams({
				page : 1,
				count : 10
			}, {
			...
			$scope.tblStStockDataItems = $scope.tblStStockDataItems - $scope.model.addSelected;
		}
	}
	$scope.removeGoodsForTransfer = function() {
		if ($scope.stToStockSelected.length <= 0) {
			...
			$scope.model.removeSelected = $scope.stToStockSelected.length;
			$scope.tblStToStockData = new NgTableParams({
				page : 1,
				count : 10
			}, {
			...
			// $scope.checkAllRemoveDefault();
			$scope.stToStockTrans = dataGoodIn;
			$scope.tblStStockDataItems = $scope.tblStStockDataItems + $scope.model.removeSelected;
	...
	$scope.getInfo = function(item) {
		...
		// internalStockId
		ctk_withdrawalOfOrders.getGoodDetail($localStorage.clientContext.shop.shopId, item.stock_id, item.goods_id, item.goods_code, item.goods_name,
