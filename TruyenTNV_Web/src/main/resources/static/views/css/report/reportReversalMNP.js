app_vnm.factory('reportReversalMNP', function($http, $translate) {
	return {
		exportFile : function(data, callback) {
			var url = eim_url + "/bs/reportReversalMNP/exportFile?reportInput=" + data;
			$http.get(url, {
...
app_vnm.controller('ctrl-reportReversalMNP', function($scope, $rootScope, $translate, $compile, $timeout, $uibModal, $location, $window, FileUploader, $filter,
		...
		if ($scope.frm_reportReversalMNP.validate()) {
			if ($scope.checkNumber()) {
				...
				var ReportInput = {
					...
					"fileTempName" : 'bao_cao_reversal_mnp',
					"fileType" : '.xlsx'
				};
