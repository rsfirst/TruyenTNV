app_vnm.factory('reportPrepaidDetailBackEnd', function($http, $translate) {
	return {
...
app_vnm.controller('ctrl-reportPrepaidDetailBackEnd', function($scope, $rootScope, $translate, $compile, $timeout, $uibModal, $location, $window, FileUploader,
		$filter, $http, NgTableParams, $localStorage, reportPrepaidDetailBackEnd) {
	...
	$scope.loadDefauld = function() {
		...
		reportPrepaidDetailBackEnd.liststaff($localStorage.clientContext.shopId, function(result) {
	...
	$scope.exportfile = function() {
		if ($scope.frm_reportPrepaidDetailBackEnd.validate()) {
			if ($scope.checkNumber()) {
				...
				var ReportInput = {
					...
					"fileTempName" : 'reportAddNewConnectPrepaidDetail_BackEnd',
					"fileType" : '.xlsx'
				};
				...
				reportPrepaidDetailBackEnd.exportFile(encodeURI(JSON.stringify(ReportInput)), function(result, status, header, config) {
					if (result.status == '0' && result.status != 'undefined') {
						App.unblockUI("#contentMain");
						bootbox.alert($translate.instant('vnm.messages.validate.prepaid.server.' + result.messages));
					} else {
