app_vnm.factory('reportCommitmentMNP', function($http, $translate) {
	return {
		exportFile : function(data, callback) {
			var url = eim_url + "/bs/reportCommitmentMNP/exportFile?reportInput=" + data;
			$http.get(url, {
...
app_vnm.controller('ctrl-reportCommitmentMNP', function($scope, $rootScope, $translate, $compile, $timeout, $uibModal, $location, $window, FileUploader, $filter,
		$http, NgTableParams, $localStorage, reportCommitmentMNP) {
//	$rootScope.checkMenu();
...
//		});
	...
	$scope.exportfile = function() {
		if ($scope.frm_reportCommitmentMNP.validate()) {
			if ($scope.checkNumber()) {
				...
				var ReportInput = {
					...
					"m_param05" : $scope.model.difStatus,
					"fileTempName" : 'bao_cao_commitment_mnp',
					"fileType" : '.xlsx'
				};
				...
				reportCommitmentMNP.exportFile(encodeURI(JSON.stringify(ReportInput)), function(result, status, header, config) {
