app_vnm.factory('reportHistorySTK', function ($http, $translate) {
    return {
        ...
        exportFile: function (data, callback) {
            var url = eim_url + "/bs/report/exportFileHistorySTK1?reportInput=" + data;
            $http.get(url, {
...
app_vnm.controller('ctrl-reportHistorySTK', function ($scope, $rootScope, $translate, $compile, $timeout, $uibModal, $location, $window, FileUploader,
                                                      $filter, $http, NgTableParams, $localStorage, reportHistorySTK) {
    ...
    $scope.exportfile = function () {
        if ($scope.frm_reportHistorySTK.validate()) {
            if ($scope.checkNumber()) {
                ...
                reportHistorySTK.listRecord(tmpMsisdn, startDate, endDate, function (result) {
                    if (result != null) {
                        $scope.listRecord = result;
                    }
                    App.unblockUI("#agentManagement");
                    if ($scope.listRecord.length == 0) {
                        reportHistorySTK.getDDK(tmpMsisdn, startDate, endDate, function (result) {
                            if (result != null) {
                                ddk = result;
                                var totalDebit = 0;
                                var totalCredit = 0;
                                ...
                                reportHistorySTK.exportFile(encodeURI(JSON.stringify(ReportInput)), function (result, status, header, config) {
                        ...
                        reportHistorySTK.getTotal(tmpMsisdn, startDate, endDate, function (result) {
                            if (result != null) {
