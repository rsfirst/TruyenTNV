app_vnm.factory('reportTransfersAumont', function($http, $translate) {
	return {
		exportFile : function(data,callback) {
			var url = eim_url + "/bs/report/reportTransfersAumont?reportInput=" + data;
			$http.get(url, {
...
app_vnm.controller('ctrl-reportTransfersAumont', function($scope, $rootScope, $translate, $compile, $timeout, $uibModal, $location, $window, FileUploader,
		$filter, $http, NgTableParams, $localStorage, reportTransfersAumont) {
...
//An Đại ca
	...
	$scope.exportfile = function() {
		...
		if ($scope.frm_reportTransfersAumont.validate()) {
			if ($scope.checkNumber()) {
				...
				var ReportInput = {
					...
					"p_todate" : $("#toDate").val(),
					"p_trans_status" : $scope.model.status,
					/*"p_email" : $scope.email, //listEmail,*/
					"p_email"  : $scope.listEmail,
					"fileTempName" : 'TemplateReportReturn006',
					"fileType" : '.xlsx'
				};
				...
				reportTransfersAumont.exportFile(encodeURI(JSON.stringify(ReportInput)), function(result, status, header, config) {
