app_vnm.factory('reportStock', function($http, $translate) {
	return {
		...
		exportFile : function(data, callback) {
			var url = eim_url + "/bs/reportStock/exportFile?reportInput=" + data;
			$http.get(url, {
...
app_vnm.controller('ctrl-reportStock', function($scope, $rootScope, $translate, $compile, $timeout, $uibModal, $location, $window, FileUploader, $filter,
		$http, NgTableParams, $localStorage, reportStock) {
	...
	$scope.loadDefauld = function() {
		...
		reportStock.listBrandedShop($localStorage.clientContext.shop.shopId,function(result) {
	...
	$scope.getStaff = function(id) {
		...
		reportStock.liststaff(id, function(result) {
			if (result.status == '0' && result.status != 'undefined') {
				App.unblockUI("#contentMain");
				bootbox.alert($translate.instant('vnm.messages.validate.prepaid.server.' + result.messages));
			} else {
	...
	$scope.exportfile = function() {
		if ($scope.frm_reportStock.validate()) {
			if ($scope.checkNumber()) {
				...
				var ReportInput = {
					...
					"fileTempName" : 'bao_cao_tong_hop_kho',
					"fileType" : '.xlsx'
				};
				...
				reportStock.exportFile(encodeURI(JSON.stringify(ReportInput)), function(result, status, header, config) {
					if (result.status == '0' && result.status != 'undefined') {
						App.unblockUI("#contentMain");
						bootbox.alert($translate.instant('vnm.messages.validate.prepaid.server.' + result.messages));
					} else {
