app_vnm.factory('reportReturnMNP', function($http, $translate) {
	return {
		exportFile : function(data, callback) {
			var url = eim_url + "/bs/reportReturnMNP/exportFile?reportInput=" + data;
			$http.get(url, {
...
app_vnm.controller('ctrl-reportReturnMNP', function($scope, $rootScope, $translate, $compile, $timeout, $uibModal, $location, $window, FileUploader, $filter,
		$http, NgTableParams, $localStorage, reportReturnMNP) {
//	$rootScope.checkMenu();
...
//		});
	...
	$scope.exportfile = function() {
		if ($scope.frm_reportReturnMNP.validate()) {
			if ($scope.checkNumber()) {
				...
				var ReportInput = {
					...
					"m_param03" : StringUtilNVL($scope.model.status),
					"m_param04" : StringUtilNVL($scope.model.receiveNetwork),					
					"m_param05" : StringUtilNVL($scope.model.originalNetwork),
					"fileTempName" : 'bao_cao_return_mnp',
					"fileType" : '.xlsx'
				};
				...
				reportReturnMNP.exportFile(encodeURI(JSON.stringify(ReportInput)), function(result, status, header, config) {
