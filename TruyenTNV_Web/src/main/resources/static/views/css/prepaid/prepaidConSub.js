app_vnm.factory('prepaidBS', function($http, $translate) {
	return {
		...
		cosNew : function(data, callback) {
			var url = eim_url + "/bs/Custome/listCOSMapNew?shopCode="+data;
			$http.post(url).success(callback).error(function(data, status, headers, config) {
				App.unblockUI("#contentMain");
				bootbox.alert(status + " " + $translate.instant('vnm.messages.error.connection'));
			});
		...
		registerBS : function(data, callback) {
			var url = eim_url + "/bs/Custome/connecterSubBS";
	...
	$scope.validationOptions = {
			...
			messages : {
				...
				mdnNew : {
					...
					maxlengthcustom : "Số điện thoại không nhiều hơn 10 số."
				},
				serialNew : {
					...
					if(result.status == '0' && result.status != 'undefined'){
						...
						prepaidBS.customer(function(result) {
							if(result.status == '0' && result.status != 'undefined'){
								...
								prepaidBS.cosNew($localStorage.clientContext.shop.shopCode,function(result) {
									if(result.status == '0' && result.status != 'undefined'){
										App.unblockUI("#contentMain");
										bootbox.alert($translate.instant('vnm.messages.validate.prepaid.server.' + result.messages));
									}else {
