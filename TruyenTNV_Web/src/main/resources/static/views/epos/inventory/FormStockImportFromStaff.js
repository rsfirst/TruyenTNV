var FORM_CODE = "ImportStaff";

app_vnm.factory('fctImportFromStaff', function($http, $translate) {
	return {
		...
		validTransActionCode : function(data, callback) {
			...
			$http.post(url, data).success(callback).error(function(data, status, headers, config) {
				closeOverLay();
				bootboxAlertFocus($translate.instant('vnm.stock_export_to_center.error.form.' + data), "fReceiptCode");
			});
		},

		getAllStockStaffSource : function(data, callback) {
			var url = eim_url + "/epos/inventory/outputForStaff/getChildsStockStaff";
			$http.post(url, data).success(callback).error(function(data, status, headers, config) {
				closeOverLay();
				bootboxAlertFocus(status + " " + $translate.instant('vnm.messages.error.connection'), "", $translate.instant("vnm.lable.vnm.name"), "");
			});
		...
		onFSaveAction : function(data, callback) {
			var url = eim_url + "/epos/inventory/outputForStaff/onSave";
			$http.post(url, data).success(callback).error(function(data, status, headers, config) {
				closeOverLay();
		...
		validData : function(data, callback) {
			var url = eim_url + "/epos/inventory/outputForStaff/validData";
			$http.post(url, data).success(callback).error(function(data, status, headers, config) {
				closeOverLay();
				bootboxAlertFocus(status + " " + $translate.instant('vnm.messages.error.connection'), "", $translate.instant("vnm.lable.vnm.name"), "");
			});
		...
		getTemplateValue : function(data, callback) {
			var url = eim_url + "/epos/inventory/stockExportToCenter/getTemplateValueImportFromStaff";
			$http.post(url, data).success(callback).error(function(data, status, headers, config) {
				closeOverLay();
				bootboxAlertFocus(status + " " + $translate.instant('vnm.messages.error.connection'), "", $translate.instant("vnm.lable.vnm.name"), "");
			});
...
app_vnm.controller('ctrl-importFromStaff', function($scope, $rootScope, $translate, $compile, $timeout, $uibModal, $location, $window, FileUploader, $filter, $http, NgTableParams, $localStorage,
		fctImportFromStaff) {

	window.document.title = $translate.instant('vnm.FormStockImportFromStaff.page.title');
	...
	$scope.checkDisabledViewStock = true;
	$scope.StockStaffSource = [];
	// lay danh sach kho nhan vien
	$scope.getAllStockStaffSource = function() {
		fctImportFromStaff.getAllStockStaffSource($localStorage.clientContext.sessionStockID, function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0') {
					bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
				} else if (result.length > 0) {
					$scope.StockStaffSource = result;
				}
			}
		});
	}
	...
	$scope.getReceiptCode = function() {
		fctImportFromStaff.getReceiptCode(function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0') {
					bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
				} else if (result.length > 0) {
	...
	$scope.loadLstGoodsForm = function() {
		...
		fctImportFromStaff.getExistedGoods(resourceCodeForm, function(result) {
	...
	//
	$scope.disableViewStockStaff = true;
	$scope.getFStockName = function() {
		if ($scope.model.stock != null && $scope.model.stock != undefined) {
			...
			$scope.disableViewStockStaff = false;
			$scope.isDisabledBtnAdd = false;
		} else {
	...
	// Button xem kho NV
	$scope.viewStockStaff = function() {
		$scope.typeViewStock = "VIEW_STOCK_STAFF";
		$scope.lstCurrentStock = [ {
			'stockId' : $scope.model.stock.stockId,
			'code' : $scope.model.stock.code,
			'name' : $scope.model.stock.name,
		}, ];
		$scope.viewStock();
	}

	// Button xem kho
	$scope.viewStockForm = function() {
		$scope.typeViewStock = "VIEW_STOCK";
		$scope.lstCurrentStock = [ {
	...
	$scope.loadDefault = function() {
		...
		$scope.model.reasonCode = $translate.instant("vnm.FormStockImportFromStaff.text.reason.value.code");
		$scope.model.reasonName = $translate.instant("vnm.FormStockImportFromStaff.text.reason.value.name");

		fctImportFromStaff.getInternalStock(function(result) {
			if (result != null && result != undefined && result.status != '0' && result.length > 0) {
				...
				fctImportFromStaff.getExistedStates(function(result) {
					if (result != null && result != undefined && result.status != '0' && result.length > 0) {
	...
	$scope.btnAddOnclick = function() {
		...
		$scope.checkDisabledViewStock = false;
	...
	$scope.btnOkOnclick = function() {
		var transGood = {
			...
			"strFromSerialClicked" : FORM_CODE,
		...
		fctImportFromStaff.onOkAction(transGood, function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0') {
	...
	$scope.onSelectGoodsForAdd = function() {
		...
		if ($scope.goodForTransactionSelected.goodsSelected == null || $scope.goodForTransactionSelected.goodsSelected == undefined || $scope.goodForTransactionSelected.goodsSelected == '') {
			$scope.model.goodsFormName = '';
		} else {
	...
	$scope.btnFSaveOnclick = function() {
		...
		var validData = {
			"receiptCode" : $scope.model.receiptCode,
			"lstGoodsTransaction" : $scope.lstGoodForTransaction,
		};
		fctImportFromStaff.validTransActionCode($scope.model.receiptCode, function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0') {
					...
					fctImportFromStaff.validData(validData, function(result) {
						if (result != null && result != undefined) {
							if (result.status == '0') {
								bootboxAlertFocus($translate.instant("vnm.output_for_staff.error.form." + result.messages), "", $translate.instant("vnm.lable.vnm.name"), "");
							} else if (result == 'ParamsOK') {
								if ($scope.disableAddChooseGoods) { // Neu dang khong them mat hang
									bootboxConfirm($translate.instant("vnm.common.btn.save"), $translate.instant("vnm.output_for_staff.confirm.messageSave"), $scope.btnFSaveOnclickConfirmOKxxx,
											$scope.btnFSaveOnclickConfirmNOKxxx);
								} else { // Neu dang them mat hang
									bootboxConfirm3($translate.instant("vnm.common.btn.save"), $translate.instant("vnm.output_for_staff.confirm.saveAddGoods"), $scope.btnFSaveOnclickConfirmOK,
	...
	$scope.btnFSaveOnclickConfirmOKxxx = function() { // chi thuc hien luu DB
		$scope.exeIEStaff();
	}

	$scope.btnFSaveOnclickConfirmNOKxxx = function() { // khong lam gi

	}

	$scope.execute = false;
	$scope.btnFSaveOnclickConfirmOK = function() { // Thuc hien them mat hang dang nhap do va luu DB
		var transGood = {
			"stockId" : $localStorage.clientContext.sessionStockID,
			"goods" : $scope.model.goodsForm,
			"internalStock" : $scope.model.stockInternalTypeForm,
	...
	$scope.btnFSaveOnclickConfirmNOK = function() { // Bo qua mat hang dang nhap do va luu DB
		$scope.exeIEStaff();
		$scope.btnCancelOnclick();
	}

	$scope.btnFSaveOnclickConfirmCancel = function() { // Khong lam gi

	}

	$scope.stockTransId = '';
	$scope.exeIEStaff = function() {
		overLoading();
		var mpStockTrans = {
			"delivererStockID" : $scope.model.stock.stockId,
			"type" : '2',
			"reasonID" : '101',// check reasonId
		...
		fctImportFromStaff.onFSaveAction(mpStockTrans, function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0') {
					bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
				} else {
					$scope.stockTransId = result;
					bootboxAlertFocus($translate.instant('vnm.FormStockImportFromStaff.SaveSuccess'), "", $translate.instant("vnm.lable.vnm.name"), "success");
	...
	$scope.btnFReInputOnclick = function() {
		...
		$scope.isDisabledBtnAdd = false;
		$scope.model.receiptCode = $scope.getReceiptCode();
		$scope.model.stock = {};
		$scope.model.note = '';
		$scope.model.fStockName = '';
		$scope.model.fStockCode = '';
		$scope.model.fStockId = '';
	...
	$scope.fValidateEmpty = function() {
		if ($scope.model.receiptCode == null || $scope.model.receiptCode == undefined || $scope.model.receiptCode == '') {
			bootboxAlertFocus($translate.instant('vnm.output_for_staff.error.form.inputReceiptCode'), 'fReceiptCode', $translate.instant("vnm.lable.vnm.name"), "");
			return false;
		}
		if ($scope.model.stock == null || $scope.model.stock == undefined || $scope.model.stock.stockId == '') {
			bootboxAlertFocus($translate.instant('vnm.output_for_staff.error.form.inputStaff'), 'fStock', $translate.instant("vnm.lable.vnm.name"), "");
			return false;
		}
		if ($scope.model.reasonCode == null || $scope.model.reasonCode == undefined || $scope.model.reasonCode == '') {
			bootboxAlertFocus($translate.instant('vnm.output_for_staff.error.form.inputReason'), 'fReasonCode', $translate.instant("vnm.lable.vnm.name"), "");
			return false;
		}
		if ($scope.lstGoodForTransaction == null || $scope.lstGoodForTransaction == undefined || $scope.lstGoodForTransaction.length <= 0) {
			bootboxAlertFocus($translate.instant('vnm.output_for_staff.error.form.inputProduct'), '', $translate.instant("vnm.lable.vnm.name"), "");
			return false;
		}
		return true;
	}
	...
	$scope.btnFPrintOnclick = function() {
		...
		fctImportFromStaff.getTemplateValue(templateValueSearch, function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0') {
					bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
				} else if (result.length > 0) {
					...
					var ReportInput = {
						"stockTranIdFromStaff" : $scope.stockTransId,
						"shopNameFromStaff" : StringUtilNVL(templateValue.shopName),
						"staffFromStaff" : StringUtilNVL(templateValue.staffName),
						"fileTempName" : 'TemplaceImportStockFromStaffReturn',
						"fileType" : '.xlsx'
					};

					fctImportFromStaff.exportFile(encodeURI(JSON.stringify(ReportInput)), function(result, status, header, config) {
	...
	$scope.createDataTableGoodsQuantitiesByGoodsId = function(goodsId) {
		...
		fctImportFromStaff.getGoodsQuantityByCondition(cond, function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0') {
					bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
				} else {
	...
	$scope.viewStock = function() {
		overLoading();
		fctImportFromStaff.getInternalStock(function(result6) {
			...
			fctImportFromStaff.getVctResourceList(sessionValue, function(result5) {
				...
				fctImportFromStaff.getExistedStates(function(result) {
					...
					fctImportFromStaff.getExistedGoodsGroups(function(result1) {
						...
						fctImportFromStaff.getExistedGoods(resourceCodeForm, function(result2) {
							...
							fctImportFromStaff.getStocksTree(shopStaff, function(result3) {
	...
	$scope.getGoodsListByCondition = function(radio) {
		var cond = {
			"stockId" : $scope.model.currentStock.stockId,
		...
		fctImportFromStaff.getGoodsListByCondition(cond, function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0') {
					bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
				} else if (result.length > 0) {
	...
	// filter hang hoa theo ma MH + ten MH
	...
	$scope.onddSearchSerialClick = function() {
		...
		fctImportFromStaff.getStockSerialByCondition(serialSearch, function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0') {
					bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
				} else if (result.length > 0) {
	...
	$scope.viewDetail = function() {
		...
		if ($scope.typeAction == 'EDIT') {
			if ($scope.itemGoodSelected.lstTransactionSerial != null && $scope.itemGoodSelected.lstTransactionSerial != undefined && $scope.itemGoodSelected.lstTransactionSerial.length > 0) {
		...
		bootbox.confirm({
			...
			callback : function(result) {
				if (result) {
					...
					// check
					fctImportFromStaff.getSerialListInStock(serialSearch, function(result) {
						if (result == null || result == undefined || result.status == '0') {
	...
	$scope.downLoadFileError = function(errorMessage) {
		...
		var objError = {
			'formName' : 'FormStockImportFromStaff',
	...
	$scope.searchSingelSerial = function() {
		...
		fctImportFromStaff.getSingelSerialInStock(serialSearch, function(result) {
			if (result.status == '0' && result.status != 'undefined') {
				App.unblockUI("#contentMain");
				bootboxAlertFocus(status + " " + $translate.instant('vnm.messages.error.connection'), "", $translate.instant("vnm.lable.vnm.name"), "");
	...
	$scope.resetDataFormStockView = function() {
		if ($scope.lstStatesForm == null || $scope.lstStatesForm == undefined || $scope.lstStatesForm.length <= 0) {
			$scope.model.stockInternalTypeForm = $scope.lstInternalStockForm[0].code;
			$scope.model.goodsForm = '';
			$scope.model.statesForm = $scope.lstStatesForm[0];
			$scope.model.goodQuantity = '';
			$scope.model.goodNote = '';
	...
	// F9 Kho nhan vien
	$("#fStockCode").keyup(function(e) {
		var code = (e.keyCode ? e.keyCode : e.which);
		if (code == '120') {
			$scope.initSearchShop = "";
			$scope.showModalFunction("modalListShop");
			$scope.itemSelectedListShop = $scope.StockStaffSource[0];
			createDataTableListShop($scope.StockStaffSource);
		} else if (code == '13') {
			$scope.onblurStockCode();
		} else {
			if ($scope.model.fStockCode == '') {
				$scope.model.fStockName = '';
				$scope.model.fStockId = '';
				$scope.disableViewStockStaff = true;
			}
	...
	$scope.initSearchShop = "";
	$scope.onblurStockCode = function() {
		if ($scope.model.fStockCode != undefined && $scope.model.fStockCode != '') {
			var isSearch = false;
			var isFirst = true;
			for (var i = 0; i < $scope.StockStaffSource.length; i++) {
				if ($scope.StockStaffSource[i].code.toUpperCase() == $scope.model.fStockCode.trim().toUpperCase()) {
					$scope.itemSelectedListShop = $scope.StockStaffSource[i];
					$scope.model.fStockCode = $scope.StockStaffSource[i].code;
					$scope.model.fStockId = $scope.StockStaffSource[i].stockId;
					$scope.model.fStockName = $scope.StockStaffSource[i].name;
					// $scope.updateFormShop();
					isSearch = true;
					$scope.isDisabledBtnAdd = false;
					break;
				} else if (isFirst && $scope.StockStaffSource[i].code.toUpperCase().includes($scope.model.fStockCode.trim().toUpperCase())) {
					$scope.itemSelectedListShop = $scope.StockStaffSource[i];
					$scope.model.fStockCode = $scope.StockStaffSource[i].code;
					$scope.model.fStockId = $scope.StockStaffSource[i].stockId;
					$scope.model.fStockName = $scope.StockStaffSource[i].name;
			...
			if (!isSearch) {
				...
				$scope.model.fStockId = '';
				$scope.isDisabledBtnAdd = true;
				$scope.initSearchShop = $scope.model.fStockCode;
				$scope.showModalFunction("modalListShop");
				createDataTableListShop($scope.StockStaffSource);
				document.getElementById('fStockCode').title = '';
				document.getElementById('fStockName').title = '';
			}
		} else {
			$scope.model.fStockName = '';
			$scope.model.fStockId = '';
	...
	function createDataTableListShop(dataItem) {
		oTableFListShop = $('#tblListShop').DataTable({
			...
			"oSearch" : {
				"sSearch" : $('#fStockCode').val().toUpperCase()
			},
			"createdRow" : function(row, data, rowIndex) {
				if ($scope.itemSelectedListShop != undefined && $scope.itemSelectedListShop != null) {
					if ($scope.itemSelectedListShop.stockId == data.stockId) {
						$(row).addClass('selected');
					}
				} else {
					if (rowIndex == 0 || rowIndex == '0') {
	...
	$scope.updateFormShop = function() {
		$scope.isDisabledBtnAdd = false;
		$scope.model.fStockCode = $scope.itemSelectedListShop.code;
		$scope.model.fStockId = $scope.itemSelectedListShop.stockId;
		$scope.model.fStockName = $scope.itemSelectedListShop.name;

		$scope.model.stock = {
			'stockId' : $scope.itemSelectedListShop.stockId,
			'code' : $scope.model.fStockCode,
			'name' : $scope.model.fStockName,
		}
		// $scope.model.stock.stockId = $scope.itemSelectedListShop.stockId;
		// $scope.model.stock.code = $scope.model.fStockCode;
		// $scope.model.stock.name = $scope.model.fStockName;

		document.getElementById('fStockCode').title = $scope.model.fStockCode;
		document.getElementById('fStockName').title = $scope.model.fStockName;
		$scope.hideModalFunction("modalListShop");
	}

	$scope.dialogListShopActionBack = function() {
		$scope.hideModalFunction("modalListShop");
		if ($scope.model.fStockName != undefined && $scope.model.fStockName.trim() == '') {
			angular.element(document.getElementById("fStockCode")).focus();
		}
	}
