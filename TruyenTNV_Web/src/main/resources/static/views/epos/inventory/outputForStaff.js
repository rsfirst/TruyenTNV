var FORM_CODE = "ExportStaff";

app_vnm.factory('fctOutputForStaff', function($http, $translate) {
	return {
		...
		getTemplateValue : function(data, callback) {
			var url = eim_url + "/epos/inventory/outputForStaff/getTemplateValue";
			$http.post(url, data).success(callback).error(function(data, status, headers, config) {
				closeOverLay();
				bootboxAlertFocus(status + " " + $translate.instant('vnm.messages.error.connection'), "", $translate.instant("vnm.lable.vnm.name"), "");
			});
...
app_vnm
		.controller(
				...
				function($scope, $rootScope, $translate, $compile, $timeout, $uibModal, $location, $window, FileUploader, $filter, $http, NgTableParams,
						$localStorage, fctOutputForStaff) {

					window.document.title = $translate.instant('vnm.output_for_staff.page.title');
					...
					$scope.getAllStockStaffSource = function() {
						fctOutputForStaff.getAllStockStaffSource($localStorage.clientContext.sessionStockID, function(result) {
					...
					$scope.getReceiptCode = function() {
						fctOutputForStaff.getReceiptCode(function(result) {
							if (result != null && result != undefined) {
								if (result.status == '0') {
									bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
								} else if (result.length > 0) {
					...
					$scope.loadLstGoodsForm = function() {
						...
						fctOutputForStaff.getExistedGoods(resourceCodeForm, function(result) {
					...
					$scope.loadDefault = function() {
						...
						$scope.model.reasonCode = $translate.instant("vnm.output_for_staff.text.reason.value.code");
						$scope.model.reasonName = $translate.instant("vnm.output_for_staff.text.reason.value.name");

						fctOutputForStaff.getInternalStock(function(result) {
							if (result != null && result != undefined && result.status != '0' && result.length > 0) {
								...
								fctOutputForStaff.getExistedStates(function(result) {
									if (result != null && result != undefined && result.status != '0' && result.length > 0) {
					...
					$scope.btnOkOnclick = function() {
						...
						fctOutputForStaff.onOkAction(transGood, function(result) {
							if (result != null && result != undefined) {
								if (result.status == '0') {
					...
					$scope.btnFSaveOnclick = function() {
						...
						fctOutputForStaff.validData(validData, function(result) {
							if (result != null && result != undefined) {
								if (result.status == '0') {
									bootboxAlertFocus($translate.instant("vnm.output_for_staff.error.form." + result.messages), "", $translate
									...
									if ($scope.disableAddChooseGoods) { // Neu dang khong them mat hang
										bootboxConfirm($translate.instant("vnm.common.btn.save"), $translate
												.instant("vnm.output_for_staff.confirm.messageSave"), $scope.btnFSaveOnclickConfirmOKxxx,
												$scope.btnFSaveOnclickConfirmNOKxxx);
									} else { // Neu dang them mat hang
										bootboxConfirm3($translate.instant("vnm.common.btn.save"), $translate
												.instant("vnm.output_for_staff.confirm.saveAddGoods"), $scope.btnFSaveOnclickConfirmOK,
					...
					$scope.exeIEStaff = function() {
						overLoading();
						var mpStockTrans = {
							"delivererStockID" : $scope.model.fStockId,
							"type" : '1',
							"reasonID" : '24',
						...
						fctOutputForStaff.onFSaveAction(mpStockTrans, function(result) {
							if (result != null && result != undefined) {
								if (result.status == '0') {
									bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
								} else {
					...
					$scope.fValidateEmpty = function() {
						if ($scope.model.receiptCode == null || $scope.model.receiptCode == undefined || $scope.model.receiptCode == '') {
							bootboxAlertFocus($translate.instant('vnm.output_for_staff.error.form.inputReceiptCode'), 'fReceiptCode', $translate
									.instant("vnm.lable.vnm.name"), "");
							return false;
						}
						if ($scope.model.fStockId == null || $scope.model.fStockId == undefined || $scope.model.fStockId.trim() == '') {
							bootboxAlertFocus($translate.instant('vnm.output_for_staff.error.form.inputStaff'), 'fStockCode', $translate
									.instant("vnm.lable.vnm.name"), "");
							return false;
						}
						if ($scope.model.reasonCode == null || $scope.model.reasonCode == undefined || $scope.model.reasonCode == '') {
							bootboxAlertFocus($translate.instant('vnm.output_for_staff.error.form.inputReason'), 'fReasonCode', $translate
									.instant("vnm.lable.vnm.name"), "");
							return false;
						}
						if ($scope.lstGoodForTransaction == null || $scope.lstGoodForTransaction == undefined || $scope.lstGoodForTransaction.length <= 0) {
					...
					$scope.btnFPrintOnclick = function() {
						...
						fctOutputForStaff.getTemplateValue($scope.stockTransId, function(result) {
							if (result != null && result != undefined) {
								if (result.status == '0') {
									bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
									closeOverLay();
									...
									var ReportInput = {
										...
										"strStaffName" : StringUtilNVL(templateValue.staffName),
										"strReportDate" : StringUtilNVL(templateValue.mstrReportDate),
										"strReportTime" : StringUtilNVL(templateValue.mstrReportTime),
										"fileTempName" : 'TemplaceDailyStockTransfer_vn',
										"fileType" : '.xlsx'
									};

									fctOutputForStaff.exportFile(encodeURI(JSON.stringify(ReportInput)), function(result, status, header, config) {
					...
					$scope.createDataTableGoodsQuantitiesByGoodsId = function(goodsId) {
						...
						fctOutputForStaff.getGoodsQuantityByCondition(cond, function(result) {
							if (result != null && result != undefined) {
								if (result.status == '0') {
									bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
								} else {
					...
					$scope.viewStock = function() {
						overLoading();
						fctOutputForStaff.getInternalStock(function(result6) {
							...
							fctOutputForStaff.getVctResourceList(sessionValue, function(result5) {
								...
								fctOutputForStaff.getExistedStates(function(result) {
									...
									fctOutputForStaff.getExistedGoodsGroups(function(result1) {
										...
										fctOutputForStaff.getExistedGoods(resourceCodeForm, function(result2) {
											...
											fctOutputForStaff.getStocksTree(shopStaff, function(result3) {
												...
												if ($scope.model.goodsForm != null && $scope.model.goodsForm != undefined
														...
														&& $scope.model.goodsForm.goodsId != '' && $scope.typeViewStock == 'VIEW_STOCK') {
													$scope.lstTableGoods = [];
												...
												if ($scope.typeViewStock == 'VIEW_STOCK_STAFF') {
													$scope.disableBtnInput = true;
												}
					...
					$scope.getGoodsListByCondition = function(radio) {
						...
						fctOutputForStaff.getGoodsListByCondition(cond, function(result) {
							if (result != null && result != undefined) {
								if (result.status == '0') {
									bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
								} else if (result.length > 0) {
					...
					$scope.onddSearchSerialClick = function() {
						...
						fctOutputForStaff.getStockSerialByCondition(serialSearch, function(result) {
							if (result != null && result != undefined) {
								if (result.status == '0') {
									bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
								} else if (result.length > 0) {
						...
						bootbox.confirm(
								{
									...
									callback : function(result) {
										if (result) {
											...
											// check
											fctOutputForStaff.getSerialListInStock(serialSearch, function(result) {
												if (result == null || result == undefined || result.status == '0') {
													message = result.messages;
													closeOverLay();
													bootboxConfirm($translate.instant("vnm.ExportStockToPartner.error.import.serial.title"), $translate
															.instant("vnm.ExportStockToPartner.error.import.serial.message"),
					...
					$scope.searchSingelSerial = function() {
						...
						fctOutputForStaff.getSingelSerialInStock(serialSearch,
								function(result) {
									if (result.status == '0' && result.status != 'undefined') {
										closeOverLay();
					...
					$("#fStockCode").keyup(function(e) {
						...
						if (code == '120') {
							...
							for (var i = 0; i < $scope.StockStaffSource.length; i++) {
								if ($scope.model.fStockCode != null && $scope.model.fStockCode != undefined &&
										($scope.StockStaffSource[i].code.toUpperCase().includes($scope.model.fStockCode.trim().toUpperCase()) ||
												$scope.StockStaffSource[i].name.toUpperCase().includes($scope.model.fStockCode.trim().toUpperCase()))) {
