...
app_vnm.controller('ctrl-stockExportToCenter', function($scope, $rootScope, $translate, $compile, $timeout, $uibModal, $location, $window, FileUploader,
		$filter, $http, NgTableParams, $localStorage, fctStockExportToCenter) {

	window.document.title = $translate.instant('vnm.convert_to_damaged_good.page.title');
	...
	$scope.loadDefault = function() {
		...
		$scope.model.reasonCode = $translate.instant("vnm.convert_to_damaged_good.text.reason.value.code");
		$scope.model.reasonName = $translate.instant("vnm.convert_to_damaged_good.text.reason.value.name");
	...
	$scope.btnOkOnclick = function() {
		...
		fctStockExportToCenter.onOkAction(transGood, function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0') {
					bootboxAlertFocus($translate.instant('vnm.convert_to_damaged_good.error.form.' + result.messages), "goodsQuantity");
				} else if (result.length > 0) {
	...
	$scope.btnFSaveOnclick = function() {
		...
		fctStockExportToCenter.validTransActionCode($scope.model.receiptCode, function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0') {
					bootboxAlertFocus($translate.instant("vnm.convert_to_damaged_good.error.form." + result.messages), "", $translate.instant("vnm.lable.vnm.name"), "");
				} else if (result == 'ParamsOK') {
					bootboxConfirm($translate.instant("vnm.common.btn.save"), $translate.instant("vnm.convert_to_damaged_good.confirm.messageSave"),
	...
	$scope.btnFSaveOnclickConfirmOK = function() {
		...
		fctStockExportToCenter.onFSaveAction(mpStockTrans, function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0') {
					var errorMsg = $translate.instant('vnm.convert_to_damaged_good.SaveError');
					bootboxAlertFocus(errorMsg + result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
				} else {
					$scope.stockTransId = result.code;
					bootboxAlertFocus($translate.instant('vnm.convert_to_damaged_good.SaveSuccess'), "", $translate.instant("vnm.lable.vnm.name"), "");
	...
	$scope.fValidateEmpty = function() {
		if ($scope.model.receiptCode == null || $scope.model.receiptCode == undefined || $scope.model.receiptCode == '') {
			bootboxAlertFocus($translate.instant('vnm.convert_to_damaged_good.error.form.inputReceiptCode'), 'fReceiptCode');
			return false;
		}
		...
		if ($scope.lstGoodForTransaction == null || $scope.lstGoodForTransaction == undefined || $scope.lstGoodForTransaction.length <= 0) {
			bootboxAlertFocus($translate.instant('vnm.stock_export_to_center.error.form.inputProduct'), '');
			return false;
		}
		
		if ($scope.model.receiptCode.length > 15) {
			bootboxAlertFocus($translate.instant('vnm.stock_export_to_center.error.form.receipt.code.max.length'), 'fReceiptCode');
			return false;
		}
			...
			if (result != null && result != undefined) {
				if (result.status == '0') {
					...
					var ReportInput = {
						...
						"fileTempName" : 'TemplateFormConvertToDamagedGoods',
						"fileType" : '.xlsx'
					};
	...
	$scope.btnSaveOnClick = function() {
		...
		if (isExist) {
			...
			bootboxAlertFocus($translate.instant('vnm.convert_to_damaged_good.error.form.choose.right.goods'), "", $translate.instant("vnm.lable.vnm.name"), "");
			$scope.disableAdd = false;
			$scope.disableAddx = false;
			$scope.disableAddViewDetail = false;
			$scope.disableAddChooseGoods = false;
	...
	$scope.onddSearchSerialClick = function() {
		if ($scope.model.ddFromSerial == null || $scope.model.ddFromSerial == undefined || $scope.model.ddFromSerial == '') {
			bootboxAlertFocus($translate.instant('vnm.convert_to_damaged_good.from.serial.empty'), "", $translate.instant("vnm.lable.vnm.name"), "");
			return;
		}
		if ($scope.model.ddToSerial == null || $scope.model.ddToSerial == undefined || $scope.model.ddToSerial == '') {
			bootboxAlertFocus($translate.instant('vnm.convert_to_damaged_good.to.serial.empty'), "", $translate.instant("vnm.lable.vnm.name"), "");
			return;
		}
		var serialSearch = {
		...
		fctStockExportToCenter.getStockSerialByCondition(serialSearch, function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0') {
					...
					bootboxAlertFocus($translate.instant('vnm.convert_to_damaged_good.error.no.data'), "", $translate.instant("vnm.lable.vnm.name"), "");
				}
			} else {
				var lstNull = [];
				$scope.fillDataDDTableSerial(lstNull);
	...
	$scope.searchSingelSerial = function() {
		...
		fctStockExportToCenter.getSingelSerialInStock(serialSearch, function(result) {
			if (result.status == '0' && result.status != 'undefined') {
				App.unblockUI("#contentMain");
				bootboxAlertFocus(status + " " + $translate.instant('vnm.messages.error.connection'), "", $translate.instant("vnm.lable.vnm.name"), "");
