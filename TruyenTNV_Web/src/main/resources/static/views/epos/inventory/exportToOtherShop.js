var FORM_CODE = "ExportAboveStock";

app_vnm.factory('fctExportToOtherShop', function($http, $translate) {
	return {
		...
		onFSaveAction : function(data, callback) {
			var url = eim_url + "/epos/inventory/exportToOtherShop/onSave";
			$http.post(url, data).success(callback).error(function(data, status, headers, config) {
				closeOverLay();
...
app_vnm
		.controller(
				'ctrl-exportToOtherShop',
				function($scope, $rootScope, $translate, $compile, $timeout, $uibModal, $location, $window, FileUploader, $filter, $http, NgTableParams,
						$localStorage, fctExportToOtherShop) {

					window.document.title = $translate.instant('vnm.export_to_other_shop.page.title');
					...
					$scope.getRelationStocksList = function() {
						fctExportToOtherShop.getRelationStocksList($localStorage.clientContext.shop.shopId, function(result) {
					...
					$scope.getReceiptCode = function() {
						fctExportToOtherShop.getReceiptCode(function(result) {
							if (result != null && result != undefined) {
								if (result.status == '0') {
									bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
								} else if (result.length > 0) {
					...
					$scope.loadLstGoodsForm = function() {
						...
						fctExportToOtherShop.getExistedGoods(resourceCodeForm, function(result) {
					...
					$scope.loadDefault = function() {
						...
						$scope.model.reasonCode = $translate.instant("vnm.export_to_other_shop.text.reason.value.code");
						$scope.model.reasonName = $translate.instant("vnm.export_to_other_shop.text.reason.value.name");

						fctExportToOtherShop.getInternalStock(function(result) {
							if (result != null && result != undefined && result.status != '0' && result.length > 0) {
								...
								fctExportToOtherShop.getVctResourceList(sessionValue, function(result) {
									if (result != null && result != undefined && result.status != '0' && result.length > 0) {
										$scope.lstGoodsResourceForm = result;
										$scope.model.resourceCodeForm = $scope.lstGoodsResourceForm[0];
										$scope.loadLstGoodsForm();

										fctExportToOtherShop.getExistedStates(function(result) {
											if (result != null && result != undefined && result.status != '0' && result.length > 0) {
												...
												fctExportToOtherShop.getReceiptCode(function(result) {
													if (result != null && result != undefined) {
														if (result.status == '0') {
															...
															fctExportToOtherShop.getRelationStocksList($localStorage.clientContext.shop.shopId,
																	function(result) {
																		if (result != null && result != undefined) {
																			if (result.status == '0') {
																				...
																				bootboxAlertFocus($translate.instant("vnm.error.noShopRelation"), "",
																						$translate.instant("vnm.lable.vnm.name"), "");
																				closeOverLay();
																				$('#pnlButtonOKAfterSave').css('display', 'none');
																				$('#pnlButtonOK').css('display', 'none');
																				$('#contentMain').find('input, textarea, button, select').attr('disabled','disabled');
																			}
																		}
																	});
														}
					...
					$scope.btnOkOnclick = function() {
						...
						fctExportToOtherShop.onOkAction(transGood, function(result) {
							if (result != null && result != undefined) {
								if (result.status == '0') {
					...
					$scope.btnFSaveOnclick = function() {
						if (!$scope.fValidateEmpty())
							return;
						fctExportToOtherShop.validTransActionCode($scope.model.receiptCode, function(result) {
							if (result != null && result != undefined) {
								if (result.status == '0') {
									bootboxAlertFocus($translate.instant("vnm.export_to_other_shop.error.form." + result.messages), "", $translate
											.instant("vnm.lable.vnm.name"), "");
								} else if (result == 'ParamsOK') {
									bootboxConfirm($translate.instant("vnm.common.btn.save"), $translate
											.instant("vnm.export_to_other_shop.confirm.messageSave"), $scope.btnFSaveOnclickConfirmOK,
					...
					$scope.btnFSaveOnclickConfirmOK = function() {
						...
						fctExportToOtherShop.onFSaveAction(mpStockTrans, function(result) {
							if (result != null && result != undefined) {
								if (result.status == '0') {
									bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
								} else {
									$scope.stockTransId = result;
									bootboxAlertFocus($translate.instant('vnm.export_to_other_shop.SaveSuccess'), "", $translate.instant("vnm.lable.vnm.name"),
											"success");
					...
					$scope.fValidateEmpty = function() {
						if ($scope.model.receiptCode == null || $scope.model.receiptCode == undefined || $scope.model.receiptCode == '') {
							bootboxAlertFocus($translate.instant('vnm.export_to_other_shop.error.form.inputReceiptCode'), 'fReceiptCode', $translate
									.instant("vnm.lable.vnm.name"), "");
							return false;
						}
						if ($scope.model.stock == null || $scope.model.stock == undefined || $scope.model.stock.stockId == '') {
							bootboxAlertFocus($translate.instant('vnm.export_to_other_shop.error.form.inputStock'), 'fStock', $translate
									.instant("vnm.lable.vnm.name"), "");
							return false;
						}
						if ($scope.model.reasonCode == null || $scope.model.reasonCode == undefined || $scope.model.reasonCode == '') {
							bootboxAlertFocus($translate.instant('vnm.export_to_other_shop.error.form.inputReason'), 'fReasonCode', $translate
									.instant("vnm.lable.vnm.name"), "");
							return false;
						}
						if ($scope.lstGoodForTransaction == null || $scope.lstGoodForTransaction == undefined || $scope.lstGoodForTransaction.length <= 0) {
							bootboxAlertFocus($translate.instant('vnm.export_to_other_shop.error.form.inputProduct'), '', $translate
									.instant("vnm.lable.vnm.name"), "");
							return false;
						}
						if ($scope.model.receiptCode.length > 15) {
							bootboxAlertFocus($translate.instant('vnm.export_to_other_shop.error.form.receipt.code.max.length'), 'fReceiptCode', $translate
									.instant("vnm.lable.vnm.name"), "");
							return false;
						}
						return true;
					...
					$scope.btnFPrintOnclick = function() {
						...
						fctExportToOtherShop.getTemplateValue(templateValueSearch, function(result) {
							if (result != null && result != undefined) {
								if (result.status == '0') {
									bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
									closeOverLay();
									...
									fctExportToOtherShop.exportFile(encodeURI(JSON.stringify(ReportInput)), function(result, status, header, config) {
					...
					$scope.createDataTableGoodsQuantitiesByGoodsId = function(goodsId) {
						...
						fctExportToOtherShop.getGoodsQuantityByCondition(cond, function(result) {
							if (result != null && result != undefined) {
								if (result.status == '0') {
									bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
								} else {
					...
					$scope.viewStock = function() {
						overLoading();
						fctExportToOtherShop.getInternalStock(function(result6) {
							...
							fctExportToOtherShop.getVctResourceList(sessionValue, function(result5) {
								...
								fctExportToOtherShop.getExistedStates(function(result) {
									...
									fctExportToOtherShop.getExistedGoodsGroups(function(result1) {
										...
										fctExportToOtherShop.getExistedGoods(resourceCodeForm, function(result2) {
											...
											fctExportToOtherShop.getStocksTree(shopStaff, function(result3) {
					...
					$scope.getGoodsListByCondition = function(radio) {
						...
						fctExportToOtherShop.getGoodsListByCondition(cond, function(result) {
							if (result != null && result != undefined) {
								if (result.status == '0') {
									bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
								} else if (result.length > 0) {
					...
					$scope.onddSearchSerialClick = function() {
						...
						fctExportToOtherShop.getStockSerialByCondition(serialSearch, function(result) {
							if (result != null && result != undefined) {
								if (result.status == '0') {
									bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
								} else if (result.length > 0) {
						...
						bootbox.confirm({
							...
							callback : function(result) {
								if (result) {
									...
									// check
									fctExportToOtherShop.getSerialListInStock(serialSearch, function(result) {
										if (result == null || result == undefined || result.status == '0') {
...
//						        var b = $(".modal-dialog").height();
					...
					$scope.searchSingelSerial = function() {
						...
						fctExportToOtherShop.getSingelSerialInStock(serialSearch,
								function(result) {
									if (result.status == '0' && result.status != 'undefined') {
										closeOverLay();
