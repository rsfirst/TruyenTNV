var IMPORT_CUSTOMER = "ImportCustomer";

app_vnm.factory('printWarrantyByBatch', function($http, $translate) {
	return {
...
/*		getExistedGoods : function( callback) {
			var url = eim_url + "/epos/inventory/stockViewInfo/getExistedGoods";
			$http.post(url).success(callback).error(function(data, status, headers, config) {
				closeOverLay();
		...
		onFSaveAction : function(data, callback) {
			var url = eim_url + "/epos/inventory/createCustomerTrans";
			$http.post(url, data).success(callback).error(function(data, status, headers, config) {
				closeOverLay();
		...
		getWarrantySerial : function(data, callback) {
			var url = eim_url + "/epos/inventory/getSerialWarrantyByBatch";
			$http.post(url, data).success(callback).error(function(data, status, headers, config) {
				closeOverLay();
				bootboxAlertFocus(status + " " + $translate.instant('vnm.messages.error.connection'),"",$translate.instant("vnm.lable.vnm.name"),"");
			});
		...
		getWarrantySerialFile : function(data, callback) {
			var url = eim_url + "/epos/inventory/getSerialListWarrantyFile";
			$http.post(url, data).success(callback).error(function(data, status, headers, config) {
				closeOverLay();
				bootboxAlertFocus(status + " " + $translate.instant('vnm.messages.error.connection'),"",$translate.instant("vnm.lable.vnm.name"),"");
			});
		...
		getChildsStockTree : function(data, callback) {
			var url = eim_url + "/epos/inventory/printwarrantybybatch/getListShop";
			$http.post(url, data).success(callback).error(function(data, status, headers, config) {
				closeOverLay();
				bootboxAlertFocus(status + " " + $translate.instant('vnm.messages.error.connection'), "", $translate.instant("vnm.lable.vnm.name"), "");
			});
...
app_vnm.controller('ctrl-printWarrantyByBatch', function($scope, $rootScope, $translate, $compile, $timeout, $uibModal, $location, $window, FileUploader,
		$filter, $http, NgTableParams, $localStorage, printWarrantyByBatch) {

	window.document.title = $translate.instant('vnm.printWarrantyByBatch.page.title');
	...
	$scope.getAllReceiveStock = function() {
		printWarrantyByBatch.getAllReceiveStock(function(result) {
	...
	var stokId = $localStorage.clientContext.sessionStaffStockID;
	// Lay ma phieu
	$scope.getReceiptCode = function() {
		printWarrantyByBatch.getReceiptCode(function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0') {
					bootboxAlertFocus(result.messages, "", $translate.instant("vnm.lable.vnm.name"), "");
				} else if (result.length > 0) {
	...
	$scope.onChooseShopForm = function() {
...
//		$scope.refreshDetailSerialForGoods();
	...
	$scope.loadLstGoodsForm = function() {
		...
		printWarrantyByBatch.getExistedGoods(resourceCodeForm, function(result) {
	...
	$scope.loadDefault = function() {
		...
		$scope.model.shop = $scope.lstShop[0];
		$scope.model.shopName = $scope.lstShop[0].shopName;*/
		
		printWarrantyByBatch.getChildsStockTree($localStorage.clientContext.shopId, function(result) {
			if (result != null && result != undefined) {
				$scope.lstShop = result;
				$scope.model.shop = $scope.lstShop[0];
				$scope.model.shopName = $scope.lstShop[0].name;
				printWarrantyByBatch.getInternalStock(function(result) {
					if (result != null && result != undefined && result.status != '0' && result.length > 0) {
						...
						printWarrantyByBatch.getVctResourceList(sessionValue, function(result) {
							if (result != null && result != undefined && result.status != '0' && result.length > 0) {
								$scope.lstGoodsResourceForm = result;
								$scope.model.resourceCodeForm = $scope.lstGoodsResourceForm[0];
								$scope.loadLstGoodsForm();

								printWarrantyByBatch.getExistedStates(function(result) {
									if (result != null && result != undefined && result.status != '0' && result.length > 0) {
	...
	$scope.confirmDeleteGoodsForTransOK = function() {
		...
		if($scope.goodForTransactionSelected != null) {
			...
			$scope.lstGoodForTransaction.splice(0, 1);
		}		
	...
	$scope.btnOkOnclick = function() {
		var transGood = {
			...
			"strFromSerialClicked" : IMPORT_CUSTOMER,
		...
		printWarrantyByBatch.onOkAction(transGood, function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0') {
					bootboxAlertFocus($translate.instant('vnm.stock_export_to_center.error.form.' + result.messages), "", $translate.instant("vnm.lable.vnm.name"), "");
...
//		$scope.onSelectGoodsForAdd();
	...
	$scope.btnFSaveOnclick = function() {
		if (!$scope.fValidateEmpty())
			return;
		printWarrantyByBatch.validTransActionCode($scope.model.receiptCode, function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0') {
					bootboxAlertFocus($translate.instant("vnm.stock_export_to_center.error.form." + result.messages), "", $translate.instant("vnm.lable.vnm.name"), "");
	...
	$scope.btnFSaveOnclickConfirmOK = function() {
		...
		printWarrantyByBatch.onFSaveAction(mpStockTrans, function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0' ) {
					...
					bootboxAlertFocus($translate.instant('vnm.printWarrantyByBatch.save.success'), "", $translate.instant("vnm.lable.vnm.name"), "success");
	...
	$scope.fValidateEmpty = function() {
		if ($scope.model.receiptCode == null || $scope.model.receiptCode == undefined || $scope.model.receiptCode == '') {
			bootboxAlertFocus($translate.instant('vnm.stock_export_to_center.error.form.inputReceiptCode'), "", $translate.instant("vnm.lable.vnm.name"), "");
			return false;
		}
		if ($scope.model.reasonCode == null || $scope.model.reasonCode == undefined || $scope.model.reasonCode == '') {
			bootboxAlertFocus($translate.instant('vnm.stock_export_to_center.error.form.inputReason'), "", $translate.instant("vnm.lable.vnm.name"), "");
			return false;
		}
		if ($scope.lstGoodForTransaction == null || $scope.lstGoodForTransaction == undefined || $scope.lstGoodForTransaction.length <= 0) {
	...
	$scope.btnFPrintOnclick = function() {
		...
		for (var i = 0; i < $scope.lstGoodForTransaction.length; i++) {
			$scope.lstSerial = $scope.lstGoodForTransaction[i].lstTransSerial;
			for (var j = 0; j < $scope.lstSerial.length; j++) {
				$scope.lstgoodEsn.push($scope.lstSerial[j].fromSerial);
				SerialString += ("'" + $scope.lstSerial[j].fromSerial + "',");
				lstSerialTmp.push($scope.lstSerial[j].fromSerial);
			}
		}
		...
		printWarrantyByBatch.getTemplateValue(templateValueSearch, function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0' || result.length <= 0) {
					...
					printWarrantyByBatch.exportFile(encodeURI(JSON.stringify(ReportInput)), function(result, status, header, config) {
					...
					printWarrantyByBatch.updateWarrantyNumber($scope.lstResult, function(result) {
						if (result.length > 0) {
							closeOverLay();
						} else {
							closeOverLay();
	...
	function createDataTableGoodsQuantities(dataItem) {
		oTableItemX = $('#tableQuantity').DataTable({
			...
			"createdRow" : function(row, data, rowIndex) {
				if ($scope.itemGoodSelected != ROW_NOT_SELECTED) {
					if ($scope.itemGoodSelected.goodsId == data.goodsId && $scope.itemGoodSelected.stateId == data.stateId) {
						$(row).addClass('selected');
					}
				} else {
					if (rowIndex == 0 || rowIndex == '0') {
	...
	$scope.createDataTableGoodsQuantitiesByGoodsId = function(goodsId) {
		...
		printWarrantyByBatch.getGoodsQuantityByCondition(cond, function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0') {
					bootboxAlertFocus(result.messages, '', $translate.instant("vnm.lable.vnm.name"), "");
				} else {
	...
	$scope.viewStockForm = function() {
		...
		$scope.lstCurrentStock = [ {
//			'stockId' : $localStorage.clientContext.shopId,
			'stockId' : $localStorage.clientContext.sessionStockID,
			'code' : $scope.model.shop.code,
			'name' : $scope.model.shop.name,
		}, ];
		$scope.viewStock();
	}
	...
	$scope.viewStock = function() {
		overLoading();
		printWarrantyByBatch.getInternalStock(function(result6) {
			...
			printWarrantyByBatch.getVctResourceList(sessionValue, function(result5) {
				...
				printWarrantyByBatch.getExistedStates(function(result) {
					...
					printWarrantyByBatch.getExistedGoodsGroups(function(result1) {
						...
						printWarrantyByBatch.getExistedGoods(resourceCodeForm, function(result2) {
							...
							/*var shopStaff = {
								"code" : $localStorage.clientContext.sessionStaffStockID,
								"name" : "",
							};*/
/*							printWarrantyByBatch.getStocksTree(shopStaff, function(result3) {
								...
								$scope.model.currentStock = $scope.model.shop;
								$scope.model.currentStockName = $scope.model.shop.name;
...
//							});
	...
	$scope.getGoodsListByCondition = function(radio) {
		...
		printWarrantyByBatch.getGoodsListByCondition(cond, function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0') {
					bootboxAlertFocus(result.messages, '', $translate.instant("vnm.lable.vnm.name"), "");
				} else if (result.length > 0) {
	...
	$scope.onddSearchSerialClick = function() {
		...
		printWarrantyByBatch.getStockSerialByCondition(serialSearch, function(result) {
			if (result != null && result != undefined) {
				if (result.status == '0') {
					bootboxAlertFocus(result.messages, '', $translate.instant("vnm.lable.vnm.name"), "");
				} else if (result.length > 0) {
	...
	$scope.viewDetail = function() {
		...
		if ($scope.typeAction == 'EDIT') {
			if ($scope.goodForTransactionSelected.lstTransSerial != null && $scope.goodForTransactionSelected.lstTransSerial != undefined
				...
				createTableDDSerialSingleSETC2($scope.goodForTransactionSelected.lstTransactionSerial);
				$scope.model.showRemoveSerial = false;
				$scope.goodForTransactionSelected.lstTransSerial.forEach(function(item) {
		...
		$scope.showModalFunction("modalSerialWarranty");
	}
	...
	var zIndexmodalSerialWarranty = $("#modalSerialWarranty").css("z-index");
	var zIndexDialogModal = $("#modalAddSerial").css("z-index");
	$scope.uploadAllFile = function(uploaderIn) {
		overLoading();
		bootboxConfirm($translate.instant("vnm.lable.vnm.name"), $translate.instant("vnm.FormStockImportFromPartner.messageAccept"), $scope.confirmUploadFileOK,
				$scope.confirmUploadFileNOK);
	}
	
	$scope.confirmUploadFileOK = function(){
		$("#modalSerialWarranty").css('zIndex', 50);
		...
		printWarrantyByBatch.getWarrantySerialFile(serialSearch, function(result) {
			if (result == null || result == undefined || result.status == '0') {
				...
				$scope.lstTableSerialsSETCTmp = []
				$scope.lstResult = result;
				Array.prototype.push.apply($scope.lstTableSerialsSETCTmp, $scope.lstResult);
				...
				$("#modalSerialWarranty").css('zIndex', zIndexmodalSerialWarranty);
	...
	$scope.confirmUploadFileNOK = function(){
		$("#modalSerialWarranty").css('zIndex', zIndexmodalSerialWarranty);
		$("#modalAddSerial").css('zIndex', zIndexDialogModal);
		closeOverLay();
	}
	...
	$scope.btnSearchSerial = function() {
		...
		printWarrantyByBatch.getWarrantySerial(serialSearch, function(result) {
			$scope.lstTableSerialsSETCTmp = [];
			if (result != null && result != undefined) {
				if (result.status == '0') {
					bootboxAlertFocus(result.messages, '', $translate.instant("vnm.lable.vnm.name"), "");
	...
	$scope.btnAcceptClick = function() {
		...
		$scope.hideModalFunction("modalSerialWarranty");
	}
	...
	$scope.showmodalSerialWarranty = function() {
		$scope.showModalFunction("modalSerialWarranty");
	}
