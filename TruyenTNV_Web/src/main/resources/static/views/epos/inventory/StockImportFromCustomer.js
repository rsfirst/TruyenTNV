...
app_vnm.factory('stockImportFromCustomer', function($http, $translate) {
	return {
		...
		getAllGoods : function(callback) {
			var url = eim_url + "/epos/inventory/getAllGoods";
			$http.post(url).success(callback).error(function(data, status, headers, config) {
				closeOverLay();
				bootboxAlertFocus(status + " " + $translate.instant('vnm.messages.error.connection'), "", $translate.instant("vnm.lable.vnm.name"), "");
			});
		...
		getStockSoldAllSerialSingle : function(data, callback) {
			var url = eim_url + "/epos/inventory/getStockSoldAllSerialSingle";
			$http.post(url, data).success(callback).error(function(data, status, headers, config) {
				closeOverLay();
				bootboxAlertFocus(status + " " + $translate.instant('vnm.messages.error.connection'), "", $translate.instant("vnm.lable.vnm.name"), "");
			});
...
app_vnm.controller('ctrl-stockImportFromCustomer',
		function($scope, $rootScope, $translate, $compile, $timeout, $uibModal, $location, $window, FileUploader, $filter, $http, NgTableParams, $localStorage,
			...
			window.document.title = $translate.instant('vnm.StockimportFromCustomer.page.title');
			...
			$scope.getAllReceiveStock = function() {
				stockImportFromCustomer.getAllReceiveStock(function(result) {
			...
			$scope.getReceiptCode = function() {
				stockImportFromCustomer.getReceiptCode(function(result) {
					if (result != null && result != undefined) {
						if (result.status == '0') {
							bootbox.alert(result.messages);
						} else if (result.length > 0) {
			...
			$scope.loadLstGoodsForm = function() {
				...
				stockImportFromCustomer.getAllGoods(function(result) {
			...
			$scope.loadDefault = function() {
				...
				$scope.model.reasonCode = $translate.instant("vnm.StockimportFromCustomer.text.reason.value.code");
				$scope.model.reasonName = $translate.instant("vnm.StockimportFromCustomer.text.reason.value.name");

				stockImportFromCustomer.getInternalStock(function(result) {
					if (result != null && result != undefined && result.status != '0' && result.length > 0) {
						...
						stockImportFromCustomer.getVctResourceList(sessionValue, function(result) {
							if (result != null && result != undefined && result.status != '0' && result.length > 0) {
								$scope.lstGoodsResourceForm = result;
								$scope.model.resourceCodeForm = $scope.lstGoodsResourceForm[0];
								$scope.loadLstGoodsForm();

								stockImportFromCustomer.getExistedStates(function(result) {
									if (result != null && result != undefined && result.status != '0' && result.length > 0) {
			...
			$scope.btnOkOnclick = function() {
				...
				stockImportFromCustomer.onOkAction(transGood, function(result) {
					if (result != null && result != undefined) {
						if (result.status == '0') {
			...
			$scope.btnFSaveOnclick = function() {
				if (!$scope.fValidateEmpty())
					return;
				stockImportFromCustomer.validTransActionCode($scope.model.receiptCode, function(result) {
					if (result != null && result != undefined) {
						if (result.status == '0') {
							bootbox.alert($translate.instant("vnm.stock_export_to_center.error.form." + result.messages));
			...
			$scope.btnFSaveOnclickConfirmOK = function() {
				...
				stockImportFromCustomer.onFSaveAction(mpStockTrans, function(result) {
					if (result != null && result != undefined) {
						if (result.status == '0') {
							...
							bootbox.alert($translate.instant('vnm.StockimportFromCustomer.save.success'));
							$scope.model.goodsForm = $scope.lstGoodForTransaction[0];
							$scope.model.goodsFormName = $scope.lstGoodForTransaction[0].name;
							for (var i = 0; i < $scope.lstStatesForm.length; i++) {
								if ($scope.lstStatesForm[i].stateId == $scope.lstGoodForTransaction[0].stateId) {
									$scope.model.statesForm = $scope.lstStatesForm[i];
								}
							}
							$scope.model.goodQuantity = $scope.lstGoodForTransaction[0].lstTransSerial.length;
							$scope.model.goodNote = $scope.lstGoodForTransaction[0].notes;
			...
			$scope.fValidateEmpty = function() {
				if ($scope.model.receiptCode == null || $scope.model.receiptCode == undefined || $scope.model.receiptCode == '') {
					bootboxAlertFocus($translate.instant('vnm.stock_export_to_center.error.form.inputReceiptCode'), 'fReceiptCode');
					return false;
				}
				if ($scope.model.reasonCode == null || $scope.model.reasonCode == undefined || $scope.model.reasonCode == '') {
					bootboxAlertFocus($translate.instant('vnm.stock_export_to_center.error.form.inputReason'), 'fReasonCode');
					return false;
				}
				return true;
			}
			...
			$scope.btnFPrintOnclick = function() {
				...
				var templateValueSearch = {
					...
					"name" : $localStorage.clientContext.sessionStaffStockID,
				}
				stockImportFromCustomer.getTemplateValue(templateValueSearch, function(result) {
					if (result != null && result != undefined) {
						if (result.status == '0') {
							bootbox.alert(result.messages);
						} else if (result.length > 0) {
							...
							stockImportFromCustomer.exportFile(encodeURI(JSON.stringify(ReportInput)), function(result, status, header, config) {
			...
			$scope.createDataTableGoodsQuantitiesByGoodsId = function(goodsId) {
				...
				stockImportFromCustomer.getGoodsQuantityByCondition(cond, function(result) {
					if (result != null && result != undefined) {
						if (result.status == '0') {
							bootbox.alert(result.messages);
						} else {
			...
			$scope.viewStock = function() {
				overLoading();
				stockImportFromCustomer.getInternalStock(function(result6) {
					...
					stockImportFromCustomer.getVctResourceList(sessionValue, function(result5) {
						...
						stockImportFromCustomer.getExistedStates(function(result) {
							...
							stockImportFromCustomer.getExistedGoodsGroups(function(result1) {
								...
								stockImportFromCustomer.getAllGoods(function(result2) {
									...
									stockImportFromCustomer.getStocksTree(shopStaff, function(result3) {
			...
			$scope.getGoodsListByCondition = function(radio) {
				...
				stockImportFromCustomer.getGoodsListByCondition(cond, function(result) {
					if (result != null && result != undefined) {
						if (result.status == '0') {
							bootbox.alert(result.messages);
						} else if (result.length > 0) {
			...
			$scope.onddSearchSerialClick = function() {
				...
				stockImportFromCustomer.getStockSerialByCondition(serialSearch, function(result) {
					if (result != null && result != undefined) {
						if (result.status == '0') {
							bootbox.alert(result.messages);
						} else if (result.length > 0) {
			...
			$scope.viewDetail = function() {
				...
				$scope.showModalFunction("modalSerialDetailSIFC");
			}
				...
				bootbox.confirm({
					...
					callback : function(result) {
						if (result) {
							...
							// check
							stockImportFromCustomer.getSerialListInStock(serialSearch, function(result) {
								if (result != null && result != undefined && result.length > 0) {
									$scope.lstSerialSETCInStock = result;
									for (var i = 0; i < $scope.fileSuccess.length; i++) {
										if ($scope.lstSerialSETCInStock.indexOf($scope.fileSuccess[i].fromSerial) < 0) {
											bootbox.alert($translate.instant('vnm.stock_view_detail.message.error.upload.fileSerial'));
											return;
										}
									}
									$scope.lstTableSerialsSETCTmp = []
									...
									$scope.disabledUploadAllFile = true;
									bootbox.alert($translate.instant('vnm.dialogButtonDetail.success').replace('%quantity', $scope.fileSuccess.length));
			...
			dowloadFile = function(data) {
				...
				$http.post(url, data, {
					...
					bootbox.alert($translate.instant('vnm.messages.validate.' + data[0]));
					closeOverLay();
				});
			}
			...
			$scope.btnSearchSerial = function() {
				...
				var serialSearch = {
					...
					"shopId" : $localStorage.clientContext.shopId,
					"strQuatity" : $scope.model.number,
				}

				stockImportFromCustomer.getStockSoldAllSerialSingle(serialSearch, function(result) {
					$scope.lstTableSerialsSETCTmp = [];
					if (result != null && result != undefined) {
						if (result.status == '0') {
							bootbox.alert(result.messages);
			...
			$scope.searchSingelSerial = function() {
				...
				stockImportFromCustomer.getSingelSerialInStock(serialSearch,
						function(result) {
							if (result.status == '0' && result.status != 'undefined') {
								App.unblockUI("#contentMain");
								...
								if (result == "") {
									bootbox.alert($translate.instant('vnm.messages.find.nodata'));
								} else {
									$scope.lstTableSerialsSETC = [];
			...
			$scope.addSerial = function() {
				...
				for (var i = 0; i < $scope.lstSerialSETCTmp.length; i++) {
					for (var j = 0; j < $scope.lstSerialSETC.length; j++) {
						if ($scope.lstSerialSETC[j].fromSerial == $scope.lstSerialSETCTmp[i].fromSerial) {
							bootbox.alert("Số Serial " + $scope.lstSerialSETCTmp[i].fromSerial + " đã có trong danh sách được chuyển");
							return;
						}
					}
				}
				...
				if ($scope.lstSerialSETCTmp.length == 0) {
					bootbox.alert("Chưa chọn Serial");
					return;
				} else {
					for (var i = 0; i < $scope.lstSerialSETCTmp.length; i++) {
				...
				for (var i = 0; i < $scope.lstTableSerialsSETCTmp.length; i++) {
					for (var j = 0; j < $scope.lstSerialSETC.length; j++) {
						if ($scope.lstSerialSETC[j].fromSerial == $scope.lstTableSerialsSETCTmp[i].fromSerial) {
							bootbox.alert("Số Serial " + $scope.lstTableSerialsSETCTmp[i].fromSerial + " đã có trong danh sách được chuyển");
						}
					}
				}
			...
			$scope.removeSerial = function() {
				...
				for (var i = 0; i < $scope.lstSerialSETC.length; i++) {
					for (var j = 0; j < $scope.lstTableSerialsSETCTmp.length; j++) {
						if ($scope.lstTableSerialsSETCTmp[j].toSerial == $scope.lstSerialSETC[i].toSerial) {
							// var index = $scope.lstSerialSETC.indexOf($scope.lstTableSerialsSETCTmp[j]);
			...
			$scope.btnAcceptClick = function() {
				if ($scope.lstSerialSETC == null || $scope.lstSerialSETC == undefined || $scope.lstSerialSETC.length <= 0) {
					bootbox.alert($translate.instant("vnm.stock_export_to_center.must.choose.serial"));
					return;
				}
				...
				$scope.hideModalFunction("modalSerialDetailSIFC");
			}
			...
			$scope.showmodalSerialDetailSIFC = function() {
				$scope.showModalFunction("modalSerialDetailSIFC");
			}
			...
			function createTableDDSerialSingleSETC(dataItem) {
				...
				$('#tblSerialSingle tbody').off().on('click', 'tr', function(e, dt, type, indexes) {
					var rowData = oTableDDSerialSingleSETC.rows(this).data();
					/*
					 * $(this).addClass('selected'); for (var i = 0; i < rowData.length; i++) { $scope.lstSerialSETCTmp.push(rowData[i]); }
					 */
					if (e.ctrlKey) {
						if (firstChoose) {
