 ...
 */
@Configuration
public class CommonUtilCSS
{

	/** The Constant LOG. */
	private static final Logger LOG = LoggerFactory.getLogger(CommonUtilCSS.class);
	...
	{
		String result = "";
		if (CommonUtilCSS.isEmpty(whereClause))
		{
	...
	{
		boolean inRange = true;
		if (CommonUtilCSS.isEmpty(gQuantity))
		{
			if (compareNumbericInString(fromValue, toValue) == true)
			{
				throw new AppException(CommonConstant.INPUT_SERIAL_ERROR_CODE, CommonConstant.INPUT_SERIAL_ERROR_DES);
