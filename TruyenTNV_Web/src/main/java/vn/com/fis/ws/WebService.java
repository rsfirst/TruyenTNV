...
public class WebService implements java.io.Serializable
{
	 ...
	 */
	public static String apiServiceMethodGET(HttpServletRequest request, String patch, String shopId, String data) throws Exception
	{
		...
		try
		{
			...
			if (session.getAttribute(IS_AUTHENTICATED) != null || !"".equals(session.getAttribute(IS_AUTHENTICATED)))
			{
				if ("1".equals(session.getAttribute(IS_AUTHENTICATED).toString()))
				{
					...
					getRequest.addHeader("Authorization", "Bearer " + transId);
					getRequest.addHeader("ShopId", shopId);
	 ...
	 */
	public static String apiServiceMethodPUTWithParam(HttpServletRequest request, String patch, String shopId, String data, HashMap<String, String> param)
			throws Exception
	{
