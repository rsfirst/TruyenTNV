...
@Service
@Transactional
public class PreOrderStatusBusiness implements PrepaidEntityFields {
    private static final Logger LOG = LoggerFactory.getLogger(PreOrderStatusBusiness.class);
    ...
    public ResponseEntity<?> searchSpecNumGroupOrderStatus(HttpServletRequest request, MnpPreOrderStatusType cosRejectModel) {
        ...
        List<MnpPreOrderStatusType> listCosReject = new ArrayList<>();
        try {
            Gson gson = new Gson();
            String dataString = gson.toJson(cosRejectModel);
            String url = ev.getProperty("login.url") + Constants.URL_STATUS_ITEM_SEARCH_GROUP_MNP_ORDER_STATUS;
            String tmp = webService.apiServiceMethodPOST(request, url, "", dataString);

            if (!"".equals(tmp)) {
                ObjectMapper jsonMapper = new ObjectMapper();
                try {
                    listCosReject = jsonMapper.readValue(tmp.toString(), new TypeReference<List<MnpPreOrderStatusType>>() {
                    });
                } catch (Exception e) {
                    try {
            ...
            LOG.info(LOG.getName() + nomeMetodo + Constants.END_LOG);
            return new ResponseEntity<List<MnpPreOrderStatusType>>(listCosReject, HttpStatus.OK);
        } catch (Exception e2) {
    ...
    public ResponseEntity<?> getListSpecNumGroupOrderStatus(HttpServletRequest request) {
        ...
        List<MnpPreOrderStatusType> listCosReject = new ArrayList<>();
        try {
            String url = ev.getProperty("login.url") + Constants.URL_STATUS_ITEM_LIST_GROUP_MNP_ORDER_STATUS;
    ...
    public ResponseEntity<?> createGroupMnpOrderStatus(HttpServletRequest request, MnpPreOrderStatusType mnpMsisdnGroupRejectModel) {
        ...
        try {
            ...
            String url = ev.getProperty("login.url") + Constants.URL_STATUS_ITEM_CREATE_GROUP_MNP_ORDER_STATUS;
            String tmp = webService.apiServiceMethodPOST(request, url, "", dataString);
    ...
    public ResponseEntity<?> onUpdateMnpOrderStatus(HttpServletRequest request, MnpPreOrderStatusType mnpMsisdnGroupRejectModel) {
        ...
        try {
            Gson gson = new Gson();
            String dataString = gson.toJson(mnpMsisdnGroupRejectModel);
            String url = ev.getProperty("login.url") + Constants.URL_STATUS_ITEM_UPDATE_GROUP_MNP_ORDER_STATUS;
            String tmp = webService.apiServiceMethodPOST(request, url, "", dataString);
    ...
    public ResponseEntity<?> deleteGroupMdnReject(HttpServletRequest request, MnpPreOrderStatusType mnpCosRejectModel) {
        ...
        try {
            ...
            String url = ev.getProperty("login.url") + Constants.URL_STATUS_ITEM_DELETE_GROUP_MNP_ORDER_STATUS;
            String tmp = webService.apiServiceMethodPOST(request, url, "", dataString);
