 ...
 */
@Service
public class PreOrderSraffLocationBusiness implements PrepaidEntityFields {
    private static final Logger LOG = LoggerFactory.getLogger(PreOrderSraffLocationBusiness.class);
    ...
    public ResponseEntity<?> getListSraffLocation(HttpServletRequest request) {
        ...
        List<StaffObject> liststaffObject = new ArrayList<>();
        try {
            String url = ev.getProperty("login.url") + Constants.URL_Staff_ITEM_LIST_GROUP_MNP;
            String tmp = webService.apiServiceMethodPOST(request, url, "", "");

            if (!"".equals(tmp)) {
                ObjectMapper jsonMapper = new ObjectMapper();
                try {
                    liststaffObject = jsonMapper.readValue(tmp.toString(), new TypeReference<List<StaffObject>>() {
                    });
                } catch (Exception e) {
                    try {
            ...
            LOG.info(LOG.getName() + nomeMetodo + Constants.END_LOG);
            return new ResponseEntity<List<StaffObject>>(liststaffObject, HttpStatus.OK);
        } catch (Exception e2) {
    ...
    public ResponseEntity<?> searchListSraffLocation(HttpServletRequest request, MnpStaffLocationType staffObject) {
        String nomeMetodo = ".onSearchStaff() ";
        LOG.info(LOG.getName() + nomeMetodo + " user: " + SecurityContextHolder.getContext().getAuthentication().getName() + Constants.BEGIN_LOG);

        JMessage jMsg = new JMessage();
        List<MnpStaffLocationType> liststaffObject = new ArrayList<>();
        try {
            Gson gson = new Gson();
            String dataString = gson.toJson(staffObject);
            String url = ev.getProperty("login.url") + Constants.URL_Staff_ITEM_SEARCH_GROUP_MNP;
            String tmp = webService.apiServiceMethodPOST(request, url, "", dataString);

            if (!"".equals(tmp)) {
                ObjectMapper jsonMapper = new ObjectMapper();
                try {
                    liststaffObject = jsonMapper.readValue(tmp.toString(), new TypeReference<List<MnpStaffLocationType>>() {
                    });
                } catch (Exception e) {
                    try {
                        jMsg = jsonMapper.readValue(tmp.toString(), new TypeReference<JMessage>() {
            ...
            LOG.info(LOG.getName() + nomeMetodo + Constants.END_LOG);
            return new ResponseEntity<List<MnpStaffLocationType>>(liststaffObject, HttpStatus.OK);
        } catch (Exception e2) {
            LOG.error(e2.getMessage(), e2);
            jMsg.setCode(Constants.COMMON_ERROR);
            jMsg.setMessage(e2.getMessage());
    ...
    public ResponseEntity<?> insertListSraffLocation(HttpServletRequest request, List<MnpStaffLocationType> staffObject) {
        ...
        try {
            Gson gson = new Gson();
            String dataString = gson.toJson(staffObject);
            String url = ev.getProperty("login.url") + Constants.URL_Staff_ITEM_INSERT_GROUP_MNP;
            String tmp = webService.apiServiceMethodPOST(request, url, "", dataString);
    ...
    public ResponseEntity<?> deleteListSraffLocation(HttpServletRequest request, List<MnpStaffLocationType> staffObject) {
        ...
        try {
            Gson gson = new Gson();
            String dataString = gson.toJson(staffObject);
            String url = ev.getProperty("login.url") + Constants.URL_Staff_ITEM_DELETE_GROUP_MNP;
            String tmp = webService.apiServiceMethodPOST(request, url, "", dataString);
